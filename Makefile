...
help:
	...
	@echo "========================================================================"
	@echo "--- BẮT ĐẦU ---"
	@echo "  make setup           - Cài deps + tạo .env (chạy 1 lần)"
	@echo ""
	@echo "--- PHÁT TRIỂN ---"
	...
	@echo "  make vps-deploy      - Deploy trên máy chủ (pull, build, restart)"
...
setup:
	pnpm install
	@test -f .env || (cp .env.example .env && echo "→ Đã tạo .env")
...
docker-size:
	...
	@docker images --format '   {{.Repository}}:{{.Tag}}\t{{.Size}}' | grep congcu || echo "   (chưa build)"
...
vps-files:
	...
	@echo ""
	@echo "  sudo mkdir -p /etc/congcu"
	@echo "  sudo cp .env.example /etc/congcu/env   # rồi điền giá trị thật"
	@echo "  sudo chmod 600 /etc/congcu/env"
	@echo ""
	@echo "  sudo cp deploy/congcu.service /etc/systemd/system/"
